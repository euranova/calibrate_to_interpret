    ...
    if type(calibrated) is str:
        ...
        if config["SCALER"]["method"] == "dirichlet":
            ...
            scores = nn.Softmax(dim=-1)(
                model(image.unsqueeze(0)) * calibrated._weights[0]
            )
    return scores
...
def save_auc_non_random(calibrated, file, name, scores_list):
    ...
    if type(calibrated) is not str:
        with open(
            "code_final/cifar100/{}/{}/{}//{}/auc_vectors_{}/auc_{}.npy".format(
                WDR, NAME_MODEL, method, file, "non_calibrated", name
            ),
            "wb",
        ) as f:
        ...
        with open(
            "code_final/cifar100/{}/{}/{}/{}/auc_vectors_{}/auc_{}.npy".format(
                WDR, NAME_MODEL, method, file, "calibrated", name
            ),
            "wb",
        ) as f:
            np.save(f, scores_list)
...
def save_auc_random(calibrated, file, name, scorelistinterp):
    if type(calibrated) is not str:
        with open(
            "code_final/cifar100/{}/{}/{}/{}/auc_vectors_random/auc_{}.npy".format(
                WDR, NAME_MODEL, method, file, "calib_" + str(name)
            ),
            "wb",
        ) as f:
        ...
        with open(
            "code_final/cifar100/{}/{}/{}/{}/auc_vectors_random/auc_{}.npy".format(
                WDR, NAME_MODEL, method, file, "non_calib" + str(name)
            ),
            "wb",
        ) as f:
            np.save(f, scorelistinterp)
...
def deletion(
    ...
    while i < borne_max:
        ...
        """plt.imshow(transforms.ToPILImage()(masked_image.detach()))
        plt.show()"""
